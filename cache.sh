...
CACHE_RETENTION_COUNT="${CACHE_RETENTION_COUNT:-2}"
CACHE_RETENTION_AGE_DAYS="${CACHE_RETENTION_AGE_DAYS:-30}"
CACHE_FALLBACK_ALLOWED="${CACHE_FALLBACK_ALLOWED:-true}"
CACHE_FALLBACK_STATUS="${CACHE_FALLBACK_STATUS:-stale}"
...
cache_store() {
        ...
        # IMPORTANT: use a DIFFERENT tmp suffix from _cache_download (which uses
        # ".tmp.$$"). If we used ".tmp.$$" too, src_file == tmp_dest and the cp
        # below would fail with "same file" error. This was the root cause of
        # run #35459743984 failure (reason=copy_failed).
        local tmp_dest="${dest}.store.$$"

        # Copy (not move) the source file to a temp path inside the cache dir.
        # We use cp (not mv) because:
        #   1. The caller may want to reuse the src (tests do this).
        ...
        if ! cp -f "$src_file" "$tmp_dest"; then
                cache_log_error "operation=store status=failed artifact=$artifact reason=copy_failed src=$src_file tmp_dest=$tmp_dest"
        ...
        if ! mv -f "$tmp_dest" "$dest"; then
                rm -f "$tmp_dest"
                cache_log_error "operation=store status=failed artifact=$artifact reason=rename_failed tmp_dest=$tmp_dest dest=$dest"
                _cache_lock_release "$fd"
                return 1
        fi
...
cache_purge_stale() {
        ...
        while IFS= read -r -d '' f; do
                case "$f" in
                        *.meta|*.tmp.*|*.store.*|*.lock|index.json) continue ;;
                esac
                b=$(basename "$f")
                if [ "$b" = "$artifact" ]; then
