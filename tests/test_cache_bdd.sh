...
test_18() {
        echo "=== Test 18: utils.sh downloader API regression ==="
        # This test catches the bug from run #35483920837 where APK downloader
        # adapter functions (get_*_resp, get_*_pkg_name, get_*_vers, dl_*) were
        # accidentally removed from utils.sh during the cache.sh refactor.
        # Without this test, the CI run goes all the way to "Build modules/APKs"
        # before failing with "command not found" — wasting minutes of build time.
        local script_dir
        script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

        # Source utils.sh (which itself sources cache.sh)
        # We need GITHUB_TOKEN placeholder + the bin/ toolchain stubs
        local TEMP_DIR="/tmp/test_18_tmp"
        local BUILD_DIR="/tmp/test_18_build"
        local BIN_DIR="/tmp/test_18_bin"
        local MODULE_TEMPLATE_DIR="/tmp/test_18_module"
        mkdir -p "$TEMP_DIR" "$BUILD_DIR" "$BIN_DIR" "$MODULE_TEMPLATE_DIR"
        GITHUB_TOKEN="" GH_HEADER="" TOML=: NEXT_VER_CODE=0 OS=Linux \
        bash -c "source '${script_dir}/utils.sh' 2>&1 | grep -v WARNING | head -1; exit 0" >/dev/null 2>&1

        # Source utils.sh in a subshell that captures function definitions
        # The simplest reliable approach: parse the file with bash -n + grep
        ...
        # Check each required function is DEFINED in utils.sh
        local required_fns=(
        ...
        # Also verify the pre-flight probe uses --patches (not -p)
        if grep -q 'list-patches --patches "\$patches_jar" --filter-package-name "com.google.android.youtube"' "$utils_file" \
           || grep -q 'list-patches --patches .\$patches_jar. --filter-package-name .com.google.android.youtube' "$utils_file"; then
                echo "    [OK] pre-flight probe uses --patches (correct option)"
                PASS=$((PASS + 1))
        else
                echo "    [FAIL] pre-flight probe does not use --patches"
                FAIL=$((FAIL + 1))
