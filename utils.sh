...
ipr() {
	# IMPORTANT: write to STDERR so stdout stays clean for callers that
	# capture get_prebuilts() stdout via $(). Run #35484821460 root cause.
	echo >&2 -e "\033[i] ${1}\033"
	if [ "${GITHUB_REPOSITORY-}" ]; then echo >&2 "::group::utils.sh [i] ${1}"; fi
}
okr() { echo >&2 -e "\033[OK] ${1}\033"; }
_clean_tmp() {
...
get_prebuilts() {
        ...
        local probe
        if probe=$(java -jar "$cli_jar" list-patches --patches "$patches_jar" -f "com.google.android.youtube" --with-versions --with-packages 2>&1); then
