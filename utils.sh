...
ipr() {
        echo -e "\033[0;36m[i] ${1}\033[0m"
        if [ "${GITHUB_REPOSITORY-}" ]; then echo -e "::group::utils.sh [i] ${1}"; fi
}
okr() { echo -e "\033[0;32m[OK] ${1}\033[0m"; }
...
get_prebuilts() {
        ...
        local probe
        if probe=$(java -jar "$cli_jar" list-patches --patches "$patches_jar" --filter-package-name "com.google.android.youtube" --versions --packages -b 2>&1); then
